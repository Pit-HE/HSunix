...
*			等待完成的功能
************************************************************/
    1：添加总线设备驱动模型里的：class
    2：添加进程间同步的 _up 和 _down 功能
    3：添加 riscv64 的原子操作接口
    4：添加 pannel 函数，处理系统中的严重错误
    5：添加内存管理中的 slab 算法
    6：添加内存管理中的伙伴算法
    7：实现有名管道的数据传输功能
    8：使用 uboot 引导并启动该操作系统内核
    9: 完成总线设备驱动模型在文件系统中创建对应文件的功能
   10：完成总线设备驱动模型中的电源管理机制
   11：完成总线设备驱动模型中上电时自动加载驱动的功能
        （添加链接文件的分区）
   12：添加进程间同步的各种上锁机制
   13：添加内核调试的日志系统
   14：实现 uboot 加载当前系统镜像的功能
   15：简单解析 uboot 启动后传给内核的参数信息以及设备树内容
