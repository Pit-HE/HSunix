...
5、内存管理：
     ...
     3）：当前操作系统用户空间中，支持小于4096字节的物理内存的动态申请与释放
          函数接口为： malloc / free 
     4）：支持类似 linux 系统中的 slab 算法来动态管理物理内存
...
11、总线设备驱动模型
	 1）：该模型是通过参考 linux 源码的设计思想而实现的，目标是可以兼容 linux 驱动
	 2）：实现 kobject 功能，可以作为基类使得继承的结构体能够调用内核的功能接口
	 3）：实现 kset 功能，可以管理 kobject 对象，也可以挂载到其他 kset 上
	 4）：实现 bus 功能，可以管理设备与驱动对象的挂载与卸载时的各种操作
	 5）：实现 device 功能，该模块主要作为内核基本的设备对象基类
	 6）：实现 driver 功能，该模块主要作为内核基本的驱动对象基类
	 7）：实现 platform 功能，可以提供基本的平台总线、设备与驱动功能，但未实现设备树解析
	 8）：实现 char_dev 功能，可以提供基本的字符设备注册、卸载和读写等基本管理功能，但还
		  未实现其在文件系统上注册具体文件对象的功能
	 9）：实现 kobj_map 功能，可以提供内核对象的哈希散列表管理功能
     10）：实现 class 功能，可以提供管理内核设备时的分类功能
...
*			等待完成的功能
************************************************************/
    1：添加进程间同步的 _up 和 _down 功能
    2：添加 riscv64 的原子操作接口
    3：添加 pannel 函数，处理系统中的严重错误
    4：简单解析 uboot 启动后传给内核的参数信息以及设备树内容
    5：实现 uboot 加载当前系统镜像的功能
    6：实现有名管道的数据传输功能
    7：使用 uboot 引导并启动该操作系统内核
    8：完成总线设备驱动模型在文件系统中创建对应文件的功能
    9: 完成总线设备驱动模型中的电源管理机制
   10：完成总线设备驱动模型中上电时自动加载驱动的功能（添加链接文件的分区）
   11：添加进程间同步的各种上锁机制
   12：添加内核调试的日志系统
